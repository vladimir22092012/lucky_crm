                ...
                if ($status_contracts) {
                    ...
                    while ($current_contract) {
                        ...
                        $this->collections->add_moving(array(
                            ...
                            'collection_status' => $current_contract->collection_status
                        ));
...
//                        $this->soap1c->send_collector($current_contract->number, $current_manager->id);
    ...
    public function check_sold_contracts()
    {
        ...
        foreach ($results as $cess) {
            $cess_numbers['Contracts'][] = $cess->number;
            $cess_numbers['Date'] = date('YmdHis', strtotime('now'));
...
//        $this->soap1c->send_cessions($cess_numbers);
                ...
                if ($results = $this->db->results()) {
                    foreach ($results as $result) {
                        ...
                        $this->collections->add_moving(array(
                            ...
                            'from_date' => date('Y-m-d H:i:s'),
                            'summ_body' => $result->loan_body_summ,
                            'summ_percents' => $result->loan_percents_summ + $result->loan_peni_summ + $result->loan_charge_summ,
                            'collection_status' => $cs_id
                        ));
                    }
                }
    ...
    public function get_contracts($filter = array(), $returnCount = false)
    {
        $id_filter = '';
        $user_id_filter = '';
        $status_filter = '';
        ...
        if (!empty($filter['search'])) {
            ...
            if (!empty($filter['search']['fio'])) {
                ...
                $fio = trim(preg_replace('/\s+/', ' ', $filter['search']['fio']));
                $fio = preg_replace('/(\w+)/', '+$1*', $fio);

                $search_filter .= $this->db->placehold(' AND (MATCH (u.lastname,u.firstname,u.patronymic) AGAINST ("' . $fio . '" IN BOOLEAN MODE))');
            }
            if (!empty($filter['search']['phone'])) {
                $users_join = 'RIGHT JOIN __users AS u ON c.user_id = u.id';
                $search_filter .= $this->db->placehold(" AND (MATCH (u.phone_mobile) AGAINST (" . $this->db->escape(str_replace(array(' ', '-', '(', ')', '+'), '', $filter['search']['phone'])) . ") ");
                $search_filter .= $this->db->placehold(" OR c.user_id IN (SELECT user_id FROM __contactpersons WHERE MATCH (phone) AGAINST (" . $this->db->escape(str_replace(array(' ', '-', '(', ')', '+'), '', $filter['search']['phone'])) . ")))");
            }
...
//    echo __FILE__.' '.__LINE__.'<br /><pre>';var_dump($query, $res);echo '</pre><hr />';
        ...
        if (isset($contract['collection_status'])) {
            if (in_array($contract['collection_status'], [3, 5, 7, 9])) {
                $this->SalesByContracts->remind_by_contract_id($id);
            }
        }
    ...
    public function contracts_closed_report_by_organization($filter = array())
    {
        $close_date_from_filter = '';
        $close_date_to_filter = '';
        ...
        if (!empty($filter['date_from'])) {
            $close_date_from_filter = $this->db->placehold("AND DATE(c.close_date) >= ?", $filter['date_from']);
            $date_from_filter = $this->db->placehold("AND DATE(op.created) >= ?", $filter['date_from']);
        }

        if (!empty($filter['date_to'])) {
            $close_date_to_filter = $this->db->placehold("AND DATE(c.close_date) <= ?", $filter['date_to']);
            $date_to_filter = $this->db->placehold("AND DATE(op.created) <= ?", $filter['date_to']);
        }

        if (!empty($filter['type_contract']))
            $contract_type_filter = $this->db->placehold("AND c.type = ?", $filter['type_contract']);
        ...
        $query = $this->db->placehold("
        select c.id, op.amount, tr.sector
        from __contracts as c
        join __operations as op on c.id = op.contract_id
    ...
    public function get_contracts_kpi($filter = array())
    {
        ...
        or close_date between ? and ?
        ", (string)$filter['return_date_from'], (string)$filter['return_date_to'], (string)$filter['return_date_from'], (string)$filter['return_date_to']);
