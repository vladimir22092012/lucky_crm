...
class Dadata extends Core
{
    ...
    public function get_onestring($query, $count = 50)
    {
        $request = array("query"=>$query, "count"=>$count);
    ...
    public function suggest($type, $fields, $customUrl = null)
    {
        if (!empty($customUrl)) {
            $this->url = $customUrl;
        }
...
//             $result = json_decode($result, true);
    ...
    public function get_city_from_ip($ip) {
        return $this->suggest('address', ['ip' => $ip], 'https://suggestions.dadata.ru/suggestions/api/4_1/rs/iplocate/');
    }
}