...
class Insurances extends Core
{
    ...
    public function get_insurance_rate($order)
    {
    	$default_rate = 0.25;
        
        // Стоимость страховки при выдаче - 25% от суммы займа по всем сегментам клиентов, в т.ч. повторники
        return $default_rate;
        ...
        $nbki_scorings = $this->scorings->get_scorings(array('type'=>'nbki', 'order_id'=>$order['order_id']));
        if (!empty($nbki_scorings))
        {
            $last_nbki_scoring = array_pop($nbki_scorings);
            $last_nbki_scoring->body = unserialize($last_nbki_scoring->body);
            
            if (isset($last_nbki_scoring->body['number_of_active'][0]))
            {
                $count_active = $last_nbki_scoring->body['number_of_active'][0];
                if ($count_active < 11)
                    return 0.2;
                else
                    return $default_rate;
