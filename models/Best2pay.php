...
7182 -> 8305 (C2A) (ФЛ) - для оплаты займа
    */
    ...
    //МИНБ
    private $sectors = array(
        'PAY_CREDIT' => '7180', //сектор для отправки кредита на карту клиента 
        'RECURRENT' => '7184', // сектор для совершения рекурентных платежей
        'ADD_CARD' => '7184', // сектор для привязки карты
        'PAYMENT' => '7182', // сектор для оплаты любой картой
        'YUK' => '7036', // подменный кабинет
        'PREMIER' => '7864', // подменный кабинет Премьер
    );
...
//        '7036' => '2L7mT49', // подменный кабинет
//        '7864' => '7q8unrA8', // подменный кабинет Премьер
//
//        '8303' => 'H16phg0',
//        '8304' => '5aF52ladm0',
//        '8305' => 'x4787V12',
    ...
    /**
     * Best2pay::reject_reason()
     * списание за причину отказа
     * @param mixed $order
     * @return
     */
    public function reject_reason($order)
    {
        ...
        $this->notify->send_reject_reason($order->order_id);
...
//        $password = $this->settings->apikeys['best2pay'][$sector];
        ...
        $description = sprintf('Оплата по договору %s. ФИО клиента: %s',
            $contract_id,
            join(' ', [$user->lastname, $user->firstname, $user->patronymic])
        );
...
//echo __FILE__.' '.__LINE__.'<br /><pre>';var_dump('$recurring', $recurring);echo '</pre><hr />';        
    ...
    public function send($method, $data, $type = 'webapi')
    {
