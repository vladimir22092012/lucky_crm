...
class Collections extends Core
{
	...
	public function get_collections($filter = array())
	{
        ...
        if (isset($filter['delay_from']))
            $delay_from = $this->db->placehold("AND delay_period >= ?", $filter['delay_from']);

        if (isset($filter['delay_to']))
            $delay_to = $this->db->placehold("AND delay_period <= ?", $filter['delay_to']);
