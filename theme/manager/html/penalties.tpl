...
{capture name='page_scripts'}
    ...
    <script type="text/javascript" src="theme/{$settings->theme|escape}/js/apps/orders.js?v=1.15"></script>
                ...
                <div class="row">
                    ...
                    <div class="col-4 ">
                        ...
                        <select name="manager_id" class="form-control js-subfilter" id="filter_manager" >
                            <option value="{url manager_id=null page=null}">Все пользователи</option>
                    ...
                    <div class="col-4 ">
                        <select class="form-control  js-subfilter" id="filter_control_manager_id" name="control_manager_id">
                            <option value="{url control_manager_id=null page=null}">Все проверяющие</option>
                            {foreach $managers as $m}
                            {if $m->role=='quality_control'}
                            <option value="{url control_manager_id=$m->id page=null}" {if $m->id==$control_manager_id}selected{/if}>{$m->name|escape}</option>
                            {/if}
                            {/foreach}
                        </select>
                    </div>
                    ...
                    <div class="card-body">
                        <div class="clearfix">
                            ...
                            <div class="float-right js-filter-client">                                
                                <a href="{if $filter_status==1}{url status=null page=null}{else}{url status='1' page=null}{/if}" class="btn btn-xs {if $filter_status==1}btn-warning{else}btn-outline-warning{/if}">Штраф</a>
                                <a href="{if $filter_status==2}{url status=null page=null}{else}{url status='2' page=null}{/if}" class="btn btn-xs {if $filter_status==2}btn-success{else}btn-outline-success{/if}">На исправление</a>
                                <a href="{if $filter_status==3}{url status=null page=null}{else}{url status='3' page=null}{/if}" class="btn btn-xs {if $filter_status==3}btn-primary{else}btn-outline-primary{/if}">Исправлен</a>
                                <a href="{if $filter_status==4}{url status=null page=null}{else}{url status='4' page=null}{/if}" class="btn btn-xs {if $filter_status==4}btn-inverse{else}btn-outline-inverse{/if}">Погашен</a>
                            </div>
                            ...
                            <div class="jsgrid-grid-header jsgrid-header-scrollbar">
                                <table class="jsgrid-table table table-striped table-hover">
                                    <tr class="jsgrid-header-row">
                                        ...
                                        <th style="width: 50px;" class=" text-right jsgrid-header-cell jsgrid-header-sortable {if $sort == 'created_asc'}jsgrid-header-sort jsgrid-header-sort-asc{elseif $sort == 'created_desc'}jsgrid-header-sort jsgrid-header-sort-desc{/if}">
                                            {if $sort == 'created_asc'}<a href="{url page=null sort='created_desc'}">Дата проверки</a>
                                            {else}<a href="{url page=null sort='created_asc'}">Дата проверки</a>{/if}
                                        </th>                                        
                                    </tr>
                                ...
                                <table class="jsgrid-table table table-striped table-hover">
                                    ...
                                    {foreach $penalties as $penalty}
                                        ...
                                        <tr class="jsgrid-row js-contract-row {if $contract->collection_workout}workout-row{/if}">
                                            <td style="width: 50px;" class="jsgrid-cell">                                                
                                                <small>{$penalty->order->date|date}</small> 
                                                <br />
                                                <small>{$penalty->order->date|time}</small>
                                            </td>
                                            {if in_array($manager->role, ['developer', 'admin', 'quality_control'])}
                                            <td style="width: 80px;" class="jsgrid-cell">
