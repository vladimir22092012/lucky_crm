...
class MailingCron extends Core
{
    ...
    private function run()
    {
        ...
        {
            if ($delayed_mailing = $this->mailings->get_delayed_item()) {
                $this->mailing($delayed_mailing);
            } elseif ($mailing = $this->mailings->get_status_item(1)) {
...
//        return null;
    ...
    private function mailing($mailing)
    {
        ...
        if (empty($client_time_warning))
        {
            ...
            {
                if ($contracts = $this->contracts->get_contracts(['user_id'=>$mailing->user_id, 'status'=>[2,4,7,10], 'sort'=>'id_desc']))
                {
                    $user_contract = reset($contracts);
                }
                ...
                if ($mailing->type == 'sms')
                {
                    ...
                    $this->communications->add_communication(array(
                        ...
                        'order_id' => empty($user_contract) ? 0 : $user_contract->order_id,
