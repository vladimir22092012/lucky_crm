...
class IssuanceCron extends Core
{
    public function __construct()
    {
        ...
        $border_low = strtotime(date('Y-m-d 00:05:00'));
        $border_high = strtotime(date('Y-m-d 23:50:00'));

        if (time() < $border_low && time() > $border_high)
        {
            file_put_contents($this->config->root_dir.'cron/log.txt', date('d-m-Y H:i:s').' Issuance SKIP'.PHP_EOL, FILE_APPEND);
            return false;
        }
                ...
                if ($res == 'APPROVED')
                {
                    ...
                    if (!empty($order->utm_source) && $order->utm_source == 'leadcraft' && !empty($order->id_1c) && !empty($order->click_hash)) {
                        if (in_array($order->client_status, ['nk', 'rep'])) {
                            try {
                                $result = $this->leadgens->send_approved_postback($order->click_hash, $order->id_1c, $contract->order_id);
                    ...
                    if (!empty($order->utm_source) && $order->utm_source == 'bankiru' && !empty($order->id_1c) && !empty($order->click_hash)) {
                        if (in_array($order->client_status, ['nk', 'rep'])) {
                            try {
                                $result = $this->leadgens->send_approved_postback_bankiru((int)$contract->order_id, $order);
                                echo __FILE__.' '.__LINE__.'<br /><pre>';var_dump($result);echo '</pre><hr />';
                            } catch (\Throwable $th) {
                            }
                        }
                    ...
                    if (!empty($order->utm_source) && $order->utm_source == 'click2money' && !empty($order->id_1c) && !empty($order->click_hash)) {
                        if (in_array($order->client_status, ['nk', 'rep'])) {
                            try {
                                $result = $this->leadgens->send_approved_postback_click2money((int)$contract->order_id, $order);
                                echo __FILE__.' '.__LINE__.'<br /><pre>';var_dump($result);echo '</pre><hr />';
                            } catch (\Throwable $th) {
                            }
                        }
                    ...
                    {
                        $insurance_rate = $this->insurances->get_insurance_rate($order);
                        $insurance_summ = round($contract->amount * $insurance_rate, 2);
...
//echo __FILE__.' '.__LINE__.'<br /><pre>';var_dump(htmlspecialchars($response));echo '</pre><hr />';
                        ...
                        if ($status == 'APPROVED')
                        {
                            ...
                            $insurance_id = $this->insurances->add_insurance(array(
                                ...
                                'end_date' => date('Y-m-d 23:59:59', time() + (30 * 86400)),
