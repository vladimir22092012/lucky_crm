...
//        $to = '2022-03-28';
        ...
        if (!empty($payments))
        {
            foreach ($payments as $item)
            {
                ...
                if (((!empty($item->ЗакрытУсловно) && empty($item->ВСуд)) || empty($item->ЗакрытУсловно)) && strlen($pay_numbers[1]) != 6)
                {
...
//echo __FILE__.' '.__LINE__.'<br /><pre>';var_dump($contract, $item);echo '</pre><hr />';
                            ...
                            if (!empty($contract->collection_manager_id) && empty($item->ЗакрытУсловно))
                            {
