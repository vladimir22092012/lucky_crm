...
//echo '<meta http-equiv="refresh" content="3">';
    ...
    private function send_cessions()
    {
        $this->db->query("
            SELECT id, number, sold_date
            FROM __contracts
            WHERE sold_sent = 0
            AND sold = 1
            AND type = 'base'
        ...
        if ($contracts = $this->db->results())
        {
            ...
            {
                $format_date = date('Ymd000000', strtotime($contract->sold_date));
                if (!isset($cessions[$format_date]))
                {
                    $cessions[$format_date] = [
                        'Date' => $format_date,
                        'Contracts' => []
                    ];
                }
                $cessions[$format_date]['Contracts'][] = $contract->number;
            }
            ...
            if (!empty($cessions))
            {
                ...
                {
                    $res = $this->soap1c->send_cessions($cession);
                    if ($res == 'OK')
                    {
                        $this->db->query("
                            UPDATE __contracts 
