...
//echo __FILE__.' '.__LINE__.'<br /><pre>';var_dump($scoring_types);echo '</pre><hr />';
            ...
            if ($order_id = $this->request->get('id', 'integer'))
            {
                if ($order = $this->orders->get_order($order_id))
                {
                    ...
                    $order->organization = $this->offline->get_organization($order->organization_id);
...
//echo __FILE__.' '.__LINE__.'<br /><pre>';var_dump($scoring_types);echo '</pre><hr />';
        ...
        foreach ($this->reasons->get_reasons(array('type'=>array('offline'))) as $r)
    ...
    private function confirm_contract_action()
    {
        ...
        $offline_point_id = $this->manager->offline_point_id;
        ...
        else
        {
            ...
            $operation_id = $this->operations->add_operation(array(
                ...
                'sent_status' => 8,  // пока не отправляем
            ...
            {
                $sms_inform_operation_id = $this->operations->add_operation(array(
                    'contract_id' => $contract->id,
                    'user_id' => $contract->user_id,
                    'order_id' => $contract->order_id,
                    'transaction_id' => 0,
                    'type' => 'SMS_INFORM',
                    'amount' => $order->sms_inform,
                    'created' => date('Y-m-d H:i:s'),
                    'sent_status' => 8, // пока не отправляем
                    'manager_id' => $this->manager->id,
                    'offline_point_id' => $offline_point_id,
            ...
            {
                $bot_inform_operation_id = $this->operations->add_operation(array(
                    'contract_id' => $contract->id,
                    'user_id' => $contract->user_id,
                    'order_id' => $contract->order_id,
                    'transaction_id' => 0,
                    'type' => 'BOT_INFORM',
                    'amount' => $order->bot_inform,
                    'created' => date('Y-m-d H:i:s'),
                    'sent_status' => 8, // пока не отправляем
                    'manager_id' => $this->manager->id,
                    'offline_point_id' => $offline_point_id,
            ...
            $this->contracts->update_contract($contract_id, array(
                ...
                'return_date' => date('Y-m-d H:i:s', time() + 86400 * $contract->period),
                'accept_code' => $code,
                'accept_date' => date('Y-m-d H:i:s'),
                'accept_ip' => $_SERVER['REMOTE_ADDR'],
                'loan_body_summ' => $contract->amount,
                'sent_status' => 0, // пока не отправляем
            ));
            ...
            if (!empty($order->sms_inform))
            {
                $params['operation'] = $this->operations->get_operation($sms_inform_operation_id);
                $this->documents->create_document(array(
                    'type' => 'OFFLINE_PKO',
                    'user_id' => $contract->user_id,
                    'order_id' => $contract->order_id,
            ...
            if (!empty($order->bot_inform))
            {
                $params['operation'] = $this->operations->get_operation($bot_inform_operation_id);
                $this->documents->create_document(array(
                    'type' => 'OFFLINE_PKO',
                    'user_id' => $contract->user_id,
                    'order_id' => $contract->order_id,
            ...
            $this->changelogs->add_changelog(array(
                ...
                'new_values' => serialize(array('status' => 5)),
                'order_id' => $contract->order_id,
                'user_id' => $contract->user_id,
            ));
    
            return array('success' => 1, 'status' => 5, 'manager' => $this->manager->name);
    ...
    private function approve_order_action()
    {
        ...
        // Создаем документы
        $this->documents->create_offline_documents($contract_id);
        
        
        return array('success'=>1, 'status'=>2, 'contract_id'=>$contract_id);
...
//echo __FILE__.' '.__LINE__.'<br /><pre>';var_dump($order);echo '</pre><hr />';        
    ...
    private function action_makepay()
    {
        $contract_id = $this->request->post('contract_id', 'integer');
        $offline_point_id = $this->request->post('offline_point_id', 'integer');
        ...
        if (empty($amount))
        {
            $this->json_output(array('error'=>'Сумма должна быть больше нуля!'));
        }
        else
        {
            if ($contract = $this->contracts->get_contract($contract_id))
            {
                ...
                $total = $contract->loan_charge_summ + $contract->loan_percents_summ + $contract->loan_body_summ + $contract->loan_peni_summ;
        
                if ($amount > $total)
                {
                    $this->json_output(array('error'=>'Обшая задолженность по договору '.$total.' руб'));
                }
                else
                {
                    $update = array();
                    ...
                    if ($amount < $total && $organization->commission > 0)
                    {
                        // Списываем комиссию
                        $rest_amount -= $organization->commission;
                        $commission = $organization->commission;;
                                            
                    }                    
                    
                    // списываем пени
                    $rest = $this->credit_debit($contract->loan_peni_summ, $rest_amount);
                    $rest_amount = $rest['debit'];
                    $update['loan_peni_summ'] = $rest['credit'];
                    
                    // списываем отв-ть
                    $rest = $this->credit_debit($contract->loan_charge_summ, $rest_amount);
                    $rest_amount = $rest['debit'];
                    $update['loan_charge_summ'] = $rest['credit'];
                    
                    // списываем проценты
                    $rest = $this->credit_debit($contract->loan_percents_summ, $rest_amount);
                    $rest_amount = $rest['debit'];
                    $update['loan_percents_summ'] = $rest['credit'];
                    
                    // списываем ОД
                    $rest = $this->credit_debit($contract->loan_body_summ, $rest_amount);
                    $rest_amount = $rest['debit'];
                    $update['loan_body_summ'] = $rest['credit'];
                    
                    // закрываем договор
                    if ($update['loan_charge_summ'] == 0 && $update['loan_percents_summ'] == 0 && $update['loan_body_summ'] == 0 && $update['loan_peni_summ'] == 0)
                    {
                    ...
                    // продлеваем
                    elseif ($update['loan_charge_summ'] == 0 && $update['loan_percents_summ'] == 0)
                    {
                        $update['return_date'] = date('Y-m-d H:i:s', time() + 86400 * $order->prolongation_period);
                        $update['status'] = 2;
                        $prolongation = 1;
                    }
                    ...
                    $transaction_id = $this->transactions->add_transaction(array(
                        ...
                        'body' => $comment,
                        'reason_code' => 1,
                        'prolongation' => empty($prolongation) ? 0 : 1,
                        'loan_charge_summ' => $contract->loan_charge_summ - $update['loan_charge_summ'],
                        'loan_percents_summ' => $contract->loan_percents_summ - $update['loan_percents_summ'],
                        'loan_body_summ' => $contract->loan_body_summ - $update['loan_body_summ'],
                        'loan_peni_summ' => $contract->loan_peni_summ - $update['loan_peni_summ'],
                        'commision_summ' => empty($commission) ? 0 : $commission,
                    ));
                    ...
                    $operation_id = $this->operations->add_operation(array(
                        ...
                        'created' => date('Y-m-d H:i:s'),
                        'sent_status' => 0, 
                        'loan_charge_summ' => $update['loan_charge_summ'],
                        'loan_percents_summ' => $update['loan_percents_summ'],
                        'loan_body_summ' => $update['loan_body_summ'],
                        'loan_peni_summ' => $update['loan_peni_summ'],
                        'manager_id' => $this->manager->id,
                        'offline_point_id' => $offline_point_id,
                    ));
                    
                    $this->transactions->update_transaction($transaction_id, array('operation_id'=>$operation_id));
                    ...
                    $comment = array(
                        ...
                        'contactperson_id' => 0,
                        'order_id' => $contract->order_id,
                        'text' => 'Внесение оплаты '.$amount.' руб по договору '.$contract->number.'. '.PHP_EOL.$comment,
                        'created' => date('Y-m-d H:i:s'),
                    );
                    ...
                    if ($transaction->loan_percents_summ > 0 || $transaction->loan_peni_summ > 0)
                    {
                        $params['pko_type'] = '1';
                        $this->documents->create_document(array(
                            'type' => 'OFFLINE_PKO',
                    ...
                    if ($transaction->loan_body_summ > 0)
                    {
                        $params['pko_type'] = '2';
                        $this->documents->create_document(array(
                            'type' => 'OFFLINE_PKO',
                    ...
                    if ($transaction->loan_charge_summ > 0)
                    {
                        $params['pko_type'] = '3';
                        $this->documents->create_document(array(
                            'type' => 'OFFLINE_PKO',
                    ...
                    if (!empty($commission))
                    {
                        $params['organization'] = $this->offline->get_organization(10); // Ежов
                        $params['pko_type'] = '50';
                        $this->documents->create_document(array(
                            'type' => 'OFFLINE_PKO',
                            'user_id' => $contract->user_id,
                    ...
                    if (!empty($close))
                    {
                        ...
                        if (!empty($email))
                            $this->send_close_contract($operation_id, $email) ;                    
                    }
                    
                    if (!empty($prolongation))
                    {
                        $prolongation_id = $this->prolongations->add_prolongation(array(
                            'contract_id' => $contract->id,
                            'user_id' => $contract->user_id,
                            'insurance_id' => empty($insurance_id) ? 0 : $insurance_id,
                            'created' => date('Y-m-d H:i:s'),
                            'return_date' => $update['return_date'],
                            'period' => $order->prolongation_period,
                            'accept_code' => '',
                            'transaction_id' => $transaction_id,
                        ));                        
                        
                        $params['prolongation'] = $this->prolongations->get_prolongation($prolongation_id);
                        
                        $this->documents->create_document(array(
                            'type' => 'OFFLINE_DOP_SOGLASHENIE',
                            'user_id' => $contract->user_id,
                            'order_id' => $contract->order_id,
                            'contract_id' => $contract->id,
                            'params' => $params,
                    ...
                    $this->changelogs->add_changelog(array(
                        ...
                        'new_values' => serialize(array('amount'=>$amount)),
                        'order_id' => $contract->order_id,
                        'user_id' => $contract->user_id,
                    ));
                    
                    
                    $this->json_output(array('success'=>'Оплата успешно принята!'));
                }
            }
            else
            {
    ...
    private function credit_debit($credit, $debit)
    {
        if ($credit >0 && $debit > 0)
        {
            if ($debit > $credit)
    ...
    private function send_sms_action()
    {
        ...
        if (!empty($order_id))
        {
            ...
            if (!empty($order->contract_id))
            {
                ...
                $template->template = $this->Sms->render_by_models($template->template, $order, $contract);
            }
        }

        $template->template = str_replace('{$firstname}', $user->firstname, $template->template);
        $template->template = $this->Sms->render_by_models($template->template, $order);
