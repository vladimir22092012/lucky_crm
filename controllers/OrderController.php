...
//echo __FILE__.' '.__LINE__.'<br /><pre>';var_dump($p2p);echo '</pre><hr />';
...
//echo __FILE__.' '.__LINE__.'<br /><pre>';var_dump($scoring_types);echo '</pre><hr />';
    ...
    private function cancel_contract_action()
    {
        $status = 6;
        $contract_id = $this->request->post('contract_id', 'integer');
        ...
        $this->orders->update_order($contract->order_id, $update);
...
//echo __FILE__.' '.__LINE__.'<br /><pre>';var_dump($order);echo '</pre><hr />';
    ...
    private function contactdata_action()
    {
        ...
        if (empty($contactdata_error)) {
            ...
            $this->changelogs->add_changelog(array(
                ...
                'order_id' => $order_id,
                'user_id' => $user_id,
                'old_passport_serial' => $order->passport_serial,
                'new_passport_serial' => $old_user->passport_serial
            ));
    ...
    private function action_correct()
    {
        $order_id = $this->request->post('order_id', 'integer');
        $comment = $this->request->post('comment');
        $od = $this->request->post('od');
        $percents = $this->request->post('percents');
