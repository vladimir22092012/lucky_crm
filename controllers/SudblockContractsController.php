...
class SudblockContractsController extends Controller
{
    public function fetch()
    {
        ...
        foreach ($this->sudblock->get_contracts($filter) as $con) {
            ...
            $loan_contracts_ids[] = $con->contract_id;

            $contracts[$con->id] = $con;
        }

        $loan_contracts = array();
        if (!empty($loan_contracts_ids)) {
            foreach ($this->contracts->get_contracts(array('id' => $loan_contracts_ids)) as $c)
                $loan_contracts[$c->id] = $c;
        }

        if (!empty($contracts)) {
            foreach ($contracts as $con) {
                if (!empty($con->contract_id) && $loan_contracts[$con->contract_id])
                    $con->loan_contract = $loan_contracts[$con->contract_id];
            }
        ...
        if ($notifications = $this->notifications->get_notifications(array('manager_id' => $this->manager->id, 'done' => 0))) {
    ...
    private function set_manager_action()
    {
        ...
        $this->notifications->add_notification(array(
            ...
            'notification_date' => date('Y-m-d H:i', strtotime('+2 day')),
            'comment' => 'Ознакомление с делом',
            'event_id' => 11
        ));
