...
class CollectorContractController extends Controller
{
    public function fetch()
    {
        ...
        if ($contract = $this->contracts->get_contract($contract_id))
        {
            ...
            $short_link = 'finfive.ru/pay/'.$code;
            ...
            if ($order = $this->orders->get_order($contract->order_id))
            {
                ...
                foreach ($this->communications->get_communications(array('user_id' => $order->user_id)) as $comm)
                {
                ...
                $this->design->assign('communication_contracts', $communication_contracts);
